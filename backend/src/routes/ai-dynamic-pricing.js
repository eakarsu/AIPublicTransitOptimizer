// AI AI-driven dynamic transit pricing
// Peak pricing to smooth ridership, maximize revenue, reduce crowding
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const response = await fetch(baseUrl + '/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPublicTransitOptimizer'
    },
    body: JSON.stringify({
      model: MODEL,
      messages: [
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AIPublicTransitOptimizer. Focus area: AI-driven dynamic transit pricing. ${`Peak pricing to smooth ridership, maximize revenue, reduce crowding`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: AI-driven dynamic transit pricing.\n${`Peak pricing to smooth ridership, maximize revenue, reduce crowding`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with the shape:\n{\n  "summary": "...",\n  "findings": ["..."],\n  "recommendations": ["..."],\n  "score": 0,\n  "confidence": 0\n}`;
    ...
    await persistResult(req.user?.id, 'dynamic-pricing', context, parsed);
    res.json({ feature: 'dynamic-pricing', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[dynamic-pricing]', err.message);
